...
/* eslint-disable */
https: var _self = 'undefined' != typeof window
    ? window
...
!(function () {
    if ('undefined' != typeof Prism && 'undefined' != typeof document) {
        var n = /<\/?(?!\d)[^\s>\/=$<%]+(?:\s(?:\s*[^\s>\/=]+(?:\s*=\s*(?:"[^"]*"|'[^']*'|[^\s'">=]+(?=[\s>]))|(?=[\s/>])))+)?\s*\/?>/g, r = /^#?((?:[\da-f]){3,4}|(?:[\da-f]{2}){3,4})$/i, o = [
            function (n) {
                var o = r.exec(n);
                if (o) {
        ...
        Prism.hooks.add('wrap', function (r) {
            if ('color' === r.type || r.classes.indexOf('color') >= 0) {
                ...
                if (!s)
                    return;
                var c = '<span class="inline-color-wrapper"><span class="inline-color" style="background-color:' +
                    s +
                    ';"></span></span>';
                r.content = c + e;
            }
